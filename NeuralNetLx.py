...
Argument:   n_x -- size of the input layer
...
"""
def initialize_parameters(n_x, n_h, n_y, seed:int = 1):
    np.random.seed(seed)
...
# def initialize_parameters(n_x, n_h, n_y, seed:int):
#     np.random.seed(seed)
#
#     W1 = np.random.randn(n_h, n_x) * 0.01
#     b1 = np.zeros((n_h, 1))
...
"""
def initialize_parameters_deep(layer_dims, seed:int = 1):
    np.random.seed(1)
    parameters = {}
...
# def initialize_parameters_deep(layer_dims,seed:int):
#     np.random.seed(seed)
#     parameters = {}
#     L = len(layer_dims)  # number of layers in the network
#
...
Returns:    A -- Post-activation parameter, of the same shape as Z.
...
Arguments:  AL -- probability vector corresponding to your label predictions, shape (1, number of examples)
...
Arguments:  X -- data, numpy array of shape (num_px * num_px * 3, number of examples)
...
def Accuracy(X, Y_lables, p_predictions):
    m = X.shape[1]
    return (np.sum(p_predictions == Y_lables) / m)
